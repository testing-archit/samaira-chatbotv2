...
export function generateStrategy(profile: Profile) {
  ...
  if (efMonths < 6 && expenses > 0) {
    ...
    issues.push(`Emergency fund is insufficient (${efMonths} months). Target: **6 months of expenses (${fmt(targetEf)})**.`);
    ...
    recommendations.push(`✅ **Emergency fund is healthy** (${efMonths} months covered).`);
  }
  ...
  const goalStrategies = (profile.goals || []).map((goal) => {
    ...
    return `**${goal.name}** (in ${goal.horizon_years} yrs, target ${fmt(target)}): Requires ~${sipLabel} at ${annualReturnPct.toFixed(1)}% assumed return.`;
  });
